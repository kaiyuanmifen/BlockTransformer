...
parser = argparse.ArgumentParser(description='Image Classification Tasks')
parser.add_argument('--model', default="functional", type=str ,help='type of transformer to use')
parser.add_argument('--data', default="cifar10", type=str, choices=("MultiMNIST",'cifar10','cifar100','pathfinder', 'MNIST', 'Triangle') ,help='data to train on')
...
elif args.data == 'MultiMNIST':
	train_dataset = DatasetMultiMNIST("../../data/double_mnist/train/")
	test_dataset = DatasetMultiMNIST("../../data/double_mnist/test/")
	OODtest_dataset = DatasetMultiMNIST("../../data/double_mnist/OODtest/")
	NoisyOODtest_dataset = DatasetMultiMNIST("../../data/double_mnist/OODtest/",Noise="Gauss")


	trainloader = torch.utils.data.DataLoader(train_dataset, batch_size = args.batch_size, num_workers = 2, shuffle = False)
	testloader = torch.utils.data.DataLoader(test_dataset, batch_size = args.batch_size, num_workers = 2, shuffle = False)
	OODtestloader= torch.utils.data.DataLoader(OODtest_dataset, batch_size = args.batch_size, num_workers = 2, shuffle = False)
	NoisyOODtestloader= torch.utils.data.DataLoader(NoisyOODtest_dataset, batch_size = args.batch_size, num_workers = 2, shuffle = False)
...
if args.model == "functional":
	...
	transformer = Vanilla_transformer_encoder(
							n_layers=args.num_layers, n_head=args.num_heads,
							 d_k=args.h_dim, d_v=args.h_dim,
							d_model=args.h_dim, d_inner=args.h_dim, dropout=args.dropout)

elif args.Method=="Block":
	transformer = Block_transformer_encoder(
	n_layers=args.num_layers, n_head=args.num_heads//4,
	d_k=args.h_dim, d_v=args.h_dim,
	d_model=args.h_dim, d_inner=args.h_dim//4, dropout=args.dropout)

elif args.Method=="Hierachy":
	transformer = Hierachy_transformer_encoder(
	n_layers=args.num_layers, n_head=args.num_heads//4,
	d_k=args.h_dim, d_v=args.h_dim,
	d_model=args.h_dim, d_inner=args.h_dim//4, dropout=args.dropout)

 
elif args.Method=="VanillaPerceiver":
...
elif args.Method=="HierachicalPerceiver":
		transformer  = Perceiver_hierachy(
		input_channels = 3,          # number of channels for each token of the input
		input_axis = 2,              # number of axis for input data (2 for images, 3 for video)
		num_freq_bands = 6,          # number of freq bands, with original value (2 * K + 1)
		max_freq = 10.,              # maximum frequency, hyperparameter depending on how fine the data is
...
class model(nn.Module):
	...
	def forward(self, img, mask = None):
		...
		if "Perceiver" not in args.Method:###perceivers have their own classifers
			x = self.mlp_head(x[:,0])
...
for epoch in range(start_epoch, start_epoch+200):
	...
	acc_OOD=test(epoch,OODtestloader)
	acc_NoisyOOD=test(epoch,NoisyOODtestloader)

	print("test_accuracy is %.3f ,OODtest_accuracy is %.3f ,NoisyOODtest_accuracy is %.3f after epochs %d"%(acc,acc_OOD,acc_NoisyOOD,epoch))
	#logging.info("test_accuracy is %.3f after epochs %d"%(acc,epoch))
	import csv 

	fields=[epoch,acc,acc_OOD,acc_NoisyOOD]
	if not os.path.isdir('checkpoint'):
		os.mkdir('checkpoint')
	with open(f'./checkpoint/'+args.name+'_log.csv', 'a') as f:
		writer = csv.writer(f)
