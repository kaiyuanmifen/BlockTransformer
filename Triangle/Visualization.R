...
AllFiles=list.files("checkpoint/")
...
for( FILE in AllFiles){
  ...
  vec=read.csv(file.path("checkpoint",FILE),header = F)
  vec$Method=strsplit(FILE,split = "[.]")[[1]][1]
  
  Data=rbind(Data,vec)
  
}
head(Data)
names(Data)=c("epoch", "acc","Method")
...
ggplot(data = Data, aes(x=epoch, y=acc,linetype=Method,colour=Method)) +
  geom_smooth(size=2)
